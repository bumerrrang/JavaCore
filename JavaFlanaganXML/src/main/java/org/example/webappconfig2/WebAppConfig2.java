package org.example.webappconfig2;
...
// getting JDOM: https://www.jdom.org/downloads/index.html
...
//        WebAppConfig2 config = new WebAppConfig2(new File(args[0]));
        WebAppConfig2 config = new WebAppConfig2(new File("./JavaFlanaganXML/src/main/resources/" + args[0]));
...
//            String name = servlet.getChild("servlet-name").getContent();
            String name = servlet.getChildText("servlet-name");
            if (name.equals(servletName)) {
                // If the names match, return the text of the <servlet-class>
//                return servlet.getChild("servlet-class").getContent();
                return servlet.getChildText("servlet-class");
            }
        }
        return null;
    }

    private static List getServlet(Element root) {
        return root.getChildren("servlet");
    }
